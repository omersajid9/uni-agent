...
#
# Requires the mini-verl submodule (python3 -m mini_verl.run below) — see examples/ssail/README.md
# for the one-time `git submodule add` step if REPO_ROOT/mini-verl doesn't exist yet.

set -xeuo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"        # examples/ssail/neuron
...
# explicitly: it embeds MODAL_TOKEN_*, which should never be committed to git (this replaces
# the old checked-in runtime_env.yaml).
if [[ -z "${RUNTIME_ENV:-}" ]]; then
  RUNTIME_ENV="$(mktemp -t ssail-neuron-runtime-env-XXXXXX.yaml)"
  cat > "${RUNTIME_ENV}" <<EOF
working_dir: ./
excludes:
  - "/.git/"
