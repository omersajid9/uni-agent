...
SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd)"           # examples/ssail/gpu
REPO_ROOT="$(cd -- "${SCRIPT_DIR}/../../.." && pwd)"
cd "${REPO_ROOT}"    # Ray packages `verl/` and `uni_agent/` from wherever this job is submitted.

# shellcheck disable=SC1091
[[ -f "${SCRIPT_DIR}/../.env" ]] && { set -a; source "${SCRIPT_DIR}/../.env"; set +a; }
: "${MODAL_TOKEN_ID:?export MODAL_TOKEN_ID, or set it in examples/ssail/.env (see examples/ssail/README.md)}"
: "${MODAL_TOKEN_SECRET:?export MODAL_TOKEN_SECRET, or set it in examples/ssail/.env (see examples/ssail/README.md)}"

project_name=${PROJECT_NAME:-"Uni-Agent-Qwen3-4B-megatron"}
exp_name=${EXP_NAME:-"$(date +%Y%m%d%H)_exp"}

# Everything data/checkpoint/log related is rooted at DATA_DIR by default, matching where
# env/setup_swe_data.sh writes the preprocessed parquet files ($HOME/uni-agent-data).
DATA_DIR=${DATA_DIR:-"$HOME/uni-agent-data"}
RUNTIME_DIR=${RUNTIME_DIR:-"${DATA_DIR}"}

# No automated downloader for this one — fetch it once, e.g.:
#   huggingface-cli download Qwen/Qwen3-4B --local-dir "${DATA_DIR}/models/Qwen3-4B"
...
TASK_CONFIG=${TASK_CONFIG:-"examples/ssail/task_config/react.yaml"}
...
# Runtime env is (re)generated into a tempfile on every run unless RUNTIME_ENV is set
# explicitly: it embeds MODAL_TOKEN_* plus this machine's conda nvidia lib paths, neither of
# which should ever be committed to git (this replaces the old checked-in runtime_env.yaml).
if [[ -z "${RUNTIME_ENV:-}" ]]; then
  CONDA_ENV_NAME=${CONDA_ENV_NAME:-uniagent}
  : "${CONDA_PREFIX:?activate the '${CONDA_ENV_NAME}' conda env first (see examples/ssail/README.md step 2)}"
  PYTHON_TAG="$(python -c 'import sys; print(f"python{sys.version_info.major}.{sys.version_info.minor}")')"
  NVIDIA_LIB_DIR="${CONDA_PREFIX}/lib/${PYTHON_TAG}/site-packages/nvidia"
  # `|| true`: under `pipefail`, grep finding zero matches (e.g. vLLM/nvidia not installed
  # yet) would otherwise trip `set -e` here even though an empty LD path is a fine fallback.
  NVIDIA_LD_PATH="$(find "${NVIDIA_LIB_DIR}" -mindepth 2 -maxdepth 2 -type d -name lib 2>/dev/null | grep -v '/cu13/' | tr '\n' ':' || true)"
  RUNTIME_ENV="$(mktemp -t ssail-gpu-runtime-env-XXXXXX.yaml)"
  cat > "${RUNTIME_ENV}" <<EOF
working_dir: ./
excludes:
  - "/.git/"
...
env_vars:
  ...
  CUDNN_HOME: "${NVIDIA_LIB_DIR}/cudnn"
  NVRTC_HOME: "${NVIDIA_LIB_DIR}/cuda_nvrtc"
  CURAND_HOME: "${NVIDIA_LIB_DIR}/curand"
  LD_LIBRARY_PATH: "${NVIDIA_LD_PATH}/usr/local/cuda/lib64"
  MAX_JOBS: "3"
