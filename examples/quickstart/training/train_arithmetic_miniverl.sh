...
RUNTIME_ENV=${RUNTIME_ENV:-"examples/quickstart/training/runtime_env_miniverl.yaml"}

# These four are re-emitted by mini_verl's own ray.init() (see ray_trainer.py), so they
# must NOT also appear in the job runtime_env yaml (that triggers a merge conflict).
# Exporting them here puts them in the driver's os.environ, which mini_verl forwards to
# the Ray workers itself. (PYTHONUNBUFFERED / RAY_DEDUP_LOGS / TQ_NUM_THREADS live in the
# yaml instead, since mini_verl does not re-emit them.)
